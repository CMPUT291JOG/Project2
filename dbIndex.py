...
# For retrieveing records, the program has one function each to implement
# 3 different methods for searching by: Key, Value, and Range
#
# This file is for testing INDEX.
#------------------------------------------------------------------------------
...
# assigning data path to sample database to filenames
	...
	while True:
		...
		# selection for retrieving records by key	
			...
			if value is None:
				...
				print('Time:', int((end - start) * (10**6)), 'microseconds')
			...
			print('Time taken:', int((end - start) * (10**6)), 'microseconds')
		...
		# selection for retrieving records by value
			...
			print()
			# if no key found, print message and continue	
			if key is None:
				print("Number of records retrieved: 0")
				print('Time:', int((end - start) * (10**6)), 'microseconds')
				print("No changes made to the answers file")
...
# function for creating the index database 
def createDB():
	...
	# set seed for randomizer to be consistent between runs
	random.seed(SEED)
	...
	for index in range(DB_SIZE):
		...
		# putting in key, value to databases		
